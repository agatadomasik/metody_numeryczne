...
if isfield(energy, country) && isfield(energy.(country), source)
    ...
    subplot(2, 1, 1);
    hold on;
    plot(x_coarse, y_yearly, 'k', 'DisplayName', 'dane wejściowe', 'LineWidth', 0.7);

    colors = ['r', 'g', 'b', 'm'];
    
    for i = 1:length(degrees)
        plot(x_fine, y_approximation{i}, 'DisplayName', ['Stopień ' num2str(degrees(i))],'Color', colors(i), 'LineWidth', 0.7);
    end
